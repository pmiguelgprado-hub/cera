// CERA — capa de UI del Atlas del sol compartido. El cálculo vive en
// calculo.js; aquí solo se lee el formulario, se pinta el acta y se
// prepara el informe imprimible.
...
// ("2549"), pero el acta debe leerse igual que el resto de cifras ("2.549").
...
// ── El sello del acta: la firma del veredicto ────────────────────────
// El texto del arco dice literalmente el resultado; el color acompaña.
// Se estampa (animación) al calcular y cuando el veredicto cambia de nivel.

const SELLO_TEXTOS = {
  verde: 'PREVIABILIDAD FAVORABLE · CERA ·',
  ambar: 'REQUIERE ESTUDIO DETALLADO · CERA',
  rojo: 'NO VIABLE CON ESTOS DATOS · CERA',
};

function pintarSello(nivel, estampar) {
  const sello = $('sello');
  sello.dataset.nivel = nivel;
  const arco = $('sello-texto');
  arco.textContent = SELLO_TEXTOS[nivel];
  arco.setAttribute('textLength', '278');
  arco.setAttribute('lengthAdjust', 'spacing');
  if (estampar && !REDUCIR.matches) {
    sello.classList.remove('estampado');
    void sello.getBoundingClientRect();
    sello.classList.add('estampado');
  }
}
...
// Concejo elegido en el atlas: contexto del caso, nunca entrada del motor.
let concejoActivo = null;

function pintarResultado(r, datos, animar = true) {
  const veredicto = $('veredicto');
  veredicto.className = `veredicto nivel-${r.semaforo.nivel}`;
  $('veredicto-texto').textContent = r.semaforo.veredicto;
  const cambioNivel = nivelAnterior !== null && nivelAnterior !== r.semaforo.nivel;
  if (cambioNivel && !REDUCIR.matches) {
    const punto = veredicto.querySelector('.punto');
    punto.classList.remove('pulso');
    void punto.offsetWidth;
    punto.classList.add('pulso');
  }
  pintarSello(r.semaforo.nivel, animar || cambioNivel);
  nivelAnterior = r.semaforo.nivel;

  // El caso es del usuario: el resumen se construye con sus propios datos,
  // y con su concejo si lo eligió en el atlas.
  $('resumen-caso').textContent =
    (concejoActivo ? `Concejo de ${concejoActivo} · ` : '') +
...
function pintarHipotesis(datos) {
  ...
  const hipotesis = [
    ...
    'El concejo elegido en el atlas es contexto informativo: el cálculo usa los escenarios prudentes comunes a toda la región.',
...
function pintarEntradasImpresion(datos) {
  ...
  const entradas = Object.entries(ETIQUETAS).map(([campo, [etiqueta, unidad]]) => {
  ...
  if (concejoActivo) {
    const div = document.createElement('div');
    const dt = document.createElement('dt');
    dt.textContent = 'Concejo (contexto del atlas)';
    const dd = document.createElement('dd');
    dd.textContent = concejoActivo;
    div.append(dt, dd);
    entradas.unshift(div);
  }
  $('print-entradas').replaceChildren(...entradas);
}

// Tras el primer cálculo válido, el acta se vuelve reactiva: cualquier
...
function ejecutar({ animar, desplazar }) {
  ...
  if (desplazar && window.matchMedia('(max-width: 979px)').matches) {
...
// Reciprocidad: la página entrega un acta desde el primer segundo,
// calculada con los datos precargados; el usuario ajusta y ve su caso.
...
form.addEventListener('focusout', (evento) => {
  ...
  if (!control.matches?.('input[type="number"], input[type="text"]')) return;
  if (control.value === '' || validar(leerFormulario())[control.name]) return;
  const marco = control.closest('.campo-unidad');
  if (!marco) return;
  marco.classList.remove('valido-destello');
  void marco.offsetWidth;
  marco.classList.add('valido-destello');
});
...
// Revelado al hacer scroll (una sola vez por elemento; sin JS o con
// movimiento reducido, todo queda visible).
if (!REDUCIR.matches && 'IntersectionObserver' in window) {
  const objetivos = document.querySelectorAll(
    '.pliego-cab, .mapa, .pasos-grid, .radio5, .comunidad-casos'
  );
...
// ── El atlas: mapa de recurso solar en el pliego I ───────────────────
...
// Barras mensuales de la ficha: SVG generado a partir de números controlados
// (sin entrada de usuario), por lo que innerHTML es seguro aquí.
function svgBarras(ey) {
  const vals = PERFIL_MENSUAL.map((f) => ey * f);
  const max = Math.max(...vals);
  const W = 240, H = 84, base = H - 12, alto = H - 24;
  const bw = W / 12;
  let s = '';
  vals.forEach((v, i) => {
    const h = (v / max) * alto;
    ...
    s += `<text x="${(x + (bw - 4) / 2).toFixed(1)}" y="${H - 2}" text-anchor="middle" class="fc-axis">${MESES[i]}</text>`;
  });
  return `<svg viewBox="0 0 ${W} ${H}" role="img" aria-label="Producción mensual por kWp">${s}</svg>`;
}

// Selección de un concejo: la ficha del atlas muestra su nombre, su zona,
// su recurso y el perfil mensual escalado.
function seleccionarConcejo(path) {
  const z = ZONAS[Number(path.dataset.zona)];
  if (!z) return;
  const nombre = concejoDe(path) ?? z.nombre;
  $('fc-nombre').textContent = nombre;
  $('fc-zona').textContent = `Zona ${z.nombre} · punto de medida: ${z.punto}`;
  animarEl($('fc-ey'), z.ey, fmt, false);
  const vs = Math.round(((z.ey - REFERENCIA_CENTRAL) / REFERENCIA_CENTRAL) * 100);
  $('fc-vs').textContent = `${vs >= 0 ? '+' : ''}${vs} %`;
  $('fc-angulo').textContent = `${z.angulo}°`;
  $('fc-barras').innerHTML = svgBarras(z.ey);
  const usar = $('fc-usar');
  usar.hidden = false;
  usar.dataset.concejo = nombre;
...
async function montarMapa() {
  ...
  try {
    ...
    // Sin mapa (fetch bloqueado o SVG corrupto): el atlas cede el sitio
    // y la calculadora sigue siendo la protagonista.
    cont.closest('.hero-mapa')?.setAttribute('hidden', '');
  }
}

montarMapa();

// «Calcular con este concejo»: dotación territorial sin tocar el motor.
// Precarga el escenario central (prudente), fija el concejo como contexto
// del acta y lleva al formulario.
$('fc-usar')?.addEventListener('click', () => {
  concejoActivo = $('fc-usar').dataset.concejo ?? null;
  const central = form.querySelector('input[name="escenario"][value="central"]');
  if (central) central.checked = true;
  if (dashboardActivo) ejecutar({ animar: false, desplazar: false });
  document.getElementById('calculadora').scrollIntoView({
    behavior: REDUCIR.matches ? 'auto' : 'smooth',
    block: 'start',
  });
  $('consumoAnualKwh').focus({ preventScroll: true });
});

// Tab bar móvil: espejo de la sección visible (scroll-spy).
const tabs = document.querySelectorAll('.tabbar a');
if (tabs.length > 0 && 'IntersectionObserver' in window) {
  const porSeccion = new Map(
    [...tabs].map((tab) => [tab.dataset.seccion, tab])
  );
  const espia = new IntersectionObserver((entradas) => {
    for (const entrada of entradas) {
      if (!entrada.isIntersecting) continue;
      for (const tab of tabs) tab.classList.remove('tab-activa');
      porSeccion.get(entrada.target.id)?.classList.add('tab-activa');
    }
  }, { rootMargin: '-40% 0px -55% 0px' });
  for (const id of porSeccion.keys()) {
...
// También al imprimir con Cmd/Ctrl+P: el acta siempre publica sus hipótesis.
